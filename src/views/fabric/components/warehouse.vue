...
<template>
	<div>
		<jod-tableList :dataSource="gridData" :columns="columns" :options="options" ref="notice"
...
<script>
	import notice from '@/api/fabric/notice.js'
	...
	export default {
		...
		methods:{
			...
			FabricList(data){
				console.log(data)
				return notice.get(this.api.fabricSupplierOrderList,data)
			},
			getList() {
			...
			//发送
			async sendOut(){
				let res = await notice.post(this.api.fabricSupplierOrderList,this.gridData)
				if(res.code==0){
					this.cancel();
